...
dict_list_orasul = {'bucuresti-ilfov 1-2 cam': 'https://www.imobiliare.ro/vanzare-apartamente/bucuresti-ilfov?id=283747521&pagina=',
                    ...
                    'cluj tot jud 3-4 cam': 'https://www.imobiliare.ro/vanzare-apartamente/cluj?id=78665944&pagina=4'}
...
item_value = [dict_page_numer_apart[input_page_numer_apart] + str(i) for i in range(number_selection, number_of_pages + 1)]
# print('key - ', item_key, ' == value - ', item_value)
item_dict = {x: y for x, y in zip(item_key, item_value)}

with open('href_imobiliare_ro.json', 'w') as fp:
    json.dump(item_dict, fp)

with open('href_imobiliare_ro.json', 'r') as fp:
...
for item_key, item_value in item_dict_json.items():
    ...
    if req.status_code == 200:
        src = req.text
        with open(fr'C:\Python mini\venv\data_imobiliare\{count}_{item_key}_IMOBILIARE_RO_{dict_camere_days[input_page_numer_apart]}_{parsing_day}.html', 'w', encoding='utf-8') as file:
            file.write(src)
        print('thats OK')
    else:
