...
parsing_day = '2022-06-12'
user_agent = UserAgent()
list_of_href_imobiliare = []
# C:\Python mini\venv\data_imobiliare\CSV imobiliare first and href\
with open(fr'C:\Python mini\venv\data_imobiliare\CSV imobiliare first and href\csv_file_{parsing_day}_list_HREF_anunt_IMOBILIARE_RO_1.csv', 'r') as file_csv:
    reader = csv.reader(file_csv)
    for i in reader:
        # print(i[0])
        list_of_href_imobiliare.append(i[1])
start_page = 6442 #int(input('Введите номер первой страницы - '))
...
item_key = [f'{i}_страница_IMOBILIARE_{parsing_day}' for i in range(start_page, number_of_pages + 1)]
...
for item_key, item_value in item_dict_json.items():
    ...
    if req.status_code == 200:
        src = req.text
        with open(fr'C:\Python mini\venv\data_IMOBILIARE_href\bucuresti_and_all\2022-06-12\{count}_страница_href_IMOBILIARE_{parsing_day}.html', 'w', encoding='utf-8') as file:
            file.write(src)
        print('thats OK')
    else:
