...
number_of_pages = 5 #int(input('Введите кол-во страниц парсинга - '))

item_key = [f'{i} страница' for i in range(number_selection, number_of_pages + 1)]
item_value = [f'https://www.imobiliare.ro/agentii/bucuresti?pagina={i}' for i in range(number_selection, number_of_pages + 1)]
# item_value = ['https://www.imobiliare.ro/dezvoltatori/bucuresti']
item_dict = {x: y for x, y in zip(item_key, item_value)}

with open('agentii_imobiliare_ro.json', 'w') as fp:
    json.dump(item_dict, fp)

with open('agentii_imobiliare_ro.json', 'r') as fp:
...
for item_key, item_value in item_dict_json.items():
    ...
    if req.status_code == 200:
        src = req.text
        with open(fr'C:\Python mini\venv\data_imobiliare_agentii\{count}_{item_key}_agentii_IMOBILIARE_RO_{parsing_day}.html', 'w', encoding='utf-8') as file:
            file.write(src)
    else:
