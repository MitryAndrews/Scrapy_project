...
with open(fr'{file_name}', 'r', encoding='utf-8') as f:
    ...
    for row in csv_file:
        ...
        # print(row['href_name'])
dict_item = {x: y for x, y in zip(list_name, list_href)}
...
for item_key, item_value in dict_item.items():
    ...
    if req.status_code == 200:
        src = req.text
        with open(fr'C:\Users\user\Scrapy_project\Archive_Vita\href_VITA_requests\{count}_{item_key}_{parsing_day}.html', 'w', encoding='utf-8') as file:
            file.write(src)
        print('thats OK')
    else:
